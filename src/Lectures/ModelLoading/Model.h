...
namespace ModelLoading
{
    ...
    public:
        ...
        {
            // Free the Resources if model is loaded already.
        ...
        /// @brief Unloads the model from memory and frees all the resources related to this model.
        void Free()
        {
            std::vector<Texture>().swap(textures_loaded);
            for (auto mesh : meshes)
            {
                mesh.Free();
                std::vector<Vertex>().swap(mesh.vertices);
                std::vector<unsigned int>().swap(mesh.indices);
                std::vector<Texture>().swap(mesh.textures);
            }
            directory = "";
        }

        /// @brief Draws the model, and thus all its meshes
        void Draw(GettingStarted::Shader& shader)
        {
    ...
    private:
        ...
        /// @brief Populates The Mesh From the Loaded aiScene & aiMesh.
